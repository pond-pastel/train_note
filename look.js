...
export function addNote(toast, insertReset, note) {
    ...
    if(timeError) {
        toast("時間が正しくないため完了できません。");
        return;
    }
    let fareError = false;
    if((note.fromStation.fare ?? 0) < 0) {
        fareError = true;
    }
    for(let i = 0;i < note.transferList.length;i++) {
        if((note.transferList[i].fare ?? 0) < 0) {
            fareError = true;
        }
    }
    if(fareError) {
        toast("料金が正しくないため完了できません。");
        return;
    };
    ...
    if(editingId !== null) {
        noteList[editingId] = note;
        cancelEdit(insertReset);
        console.log("ノートを編集しました。");
    } else {
        noteList.push(note);
...
function editNote(toast, applyInsert, id) {
    ...
    const addNoteEle = document.getElementById("add-note");
    addNoteEle.textContent = "編集完了";
    const resetButtonEle = document.getElementById("reset-button");
    resetButtonEle.textContent = "編集をキャンセル";
    ...
    data.insertData = noteList[id];
    localStorage.setItem("train_note", JSON.stringify(data));
    applyInsert(toast);
}

export function cancelEdit(insertReset) {
    if(editingId === null) {
        return;
    }
    editingId = null;
    const addNoteEle = document.getElementById("add-note");
    addNoteEle.textContent = "+ノートを追加";
    const resetButtonEle = document.getElementById("reset-button");
    resetButtonEle.textContent = "ノートをリセット";
    if(insertReset) {
        insertReset();
    }
}

function deleteNote(toast, applyInsert, id) {
    if(sortList ?? searchList) {
        toast("エラーが発生しました。");
        return;
    }
...
export function lookNote(toast, applyInsert, look = true) {
    ...
    for(let i = 0;i < list.length;i++) {
        ...
        if(!(sortList ?? searchList)) {
            const editBox = document.createElement("p");
            editBox.classList.add("edit-box");
            const editButton = document.createElement("button");
            editButton.textContent = "編集";
            editButton.classList.add("edit-button");
            editButton.addEventListener("click", () => {
                editNote(toast, applyInsert, i);
            });
            editBox.append(editButton);
            ulBox.append(editBox);
            ...
            deleteButton.addEventListener("click", () => {
                console.log(i);
                toast(`${t}を削除しますか？`, deleteNote, [toast, applyInsert, i]);
            });
...
export function search(toast, applyInsert) {
    console.log("search");
...
export function sort(toast, applyInsert) {
